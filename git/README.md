...
4. This chunk start with "First line". It doesn't start with "-" or "+", which means it stay unchanged. It's showed in this chunk as a marker.
5. Line "Second line" was from `a`.
6. Lines "2nd line" and "3rd line" were from `b`. Which means this 2 lines replace the "Second line" in `a`.
...
6. Force overwrite remote ref: add `-f` to `push` command. This usually happens when a local branch was rolled back a few commits and then push to the remote.
7. Change remote URL: `git remote set-url origin <new_url>`.
